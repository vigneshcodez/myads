...
from . models import Category,Business,Review,Advertisement,Location,ZodiacDailyMessage , News,RelatedAdvertisement,Tourism,BusinessBlog
...
def all_categories(request):
    categories = Category.objects.filter(parent__isnull=True)
    location = Location.objects.all()
    return render(request,'app/pages/all_categories.html',{'categories':categories,'location': location})
...
def category_detail(request, **kwargs):
    ...
    if not subcategories:
        ...
        ads = RelatedAdvertisement.objects.filter(category_id=category)
        for i in page_obj:
            average_rating = i.reviews.aggregate(avg=Avg('rating'))['avg'] or 0
            i.rating = average_rating
...
def tourism(request,slug):
    blog = Tourism.objects.get(slug=slug)
    return render(request,'app/pages/tourism.html',{'blog':blog})
