...
class Business(models.Model, ImageCleanupMixin):
    ...
    def clean(self):
        if not self.category.is_leaf_node():
            raise ValidationError("Only leaf categories are allowed for businesses.")
...
class RelatedAdvertisement(models.Model):
    banner_image = models.ImageField(upload_to='related_advertisement_images/', null=False, blank=False, verbose_name="Banner Image")
    category = models.ForeignKey(Category, on_delete=models.CASCADE, verbose_name="Business Category")
    advertised_business = models.ForeignKey(Business,on_delete=models.CASCADE)
...
class Tourism(models.Model):
    ...
    banner_image = models.ImageField(upload_to='tourism_images/', null=False, blank=False, verbose_name="Banner Image")
    content = HTMLField()
    ...
    def save(self, *args, **kwargs):
        if not self.slug:
            ...
            unique_slug = base_slug
            counter = 1
            while Tourism.objects.filter(slug=unique_slug).exists():
                unique_slug = f"{base_slug}-{counter}"
                counter += 1
            self.slug = unique_slug
        super().save(*args, **kwargs)
